...
object tony_exercises {
  ...
  object List {
    def foldRight[A, B](as: List[A], b: B, f: (A, B) => B): B = as match {
	  ...
	  };import scala.runtime.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(699); 
	 
	  def foldLeft[A, B](as: List[A], b: B, f: (B, A) => B): B = as match {
	    case Empty => b
	    case Cons(h, t) => foldLeft(t, f(b, h), f)
	  };System.out.println("""foldLeft: [A, B](as: tony_exercises.List[A], b: B, f: (B, A) => B)B""");$skip(178); 
	  ...
	  };System.out.println("""reduceRight: [A](as: tony_exercises.List[A], f: (A, A) => A)A""");$skip(175); 
	 
	  def reduceLeft[A](as: List[A], f: (A, A) => A): A = as match {
	    case Empty => error("bzzt. reduceLeft on empty list")
	    case Cons(h, t) => foldLeft(t, h, f)
	  };System.out.println("""reduceLeft: [A](as: tony_exercises.List[A], f: (A, A) => A)A""");$skip(160); 
	 
	  def unfold[A, B](b: B, f: B => Option[(A, B)]): List[A] = f(b) match {
	    case Some((a, b)) => Cons(a, unfold(b, f))
	    case scala.None => Empty
	  };System.out.println("""unfold: [A, B](b: B, f: B => Option[(A, B)])tony_exercises.List[A]""")}
	}
